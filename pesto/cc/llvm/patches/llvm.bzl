...
def gentbl(name, tblgen, td_file, td_srcs, tbl_outs, library=True, **kwargs):
    ...
    for (opts, out) in tbl_outs:
        ...
        rule_suffix = "_".join(
            opts.replace("-", "_").replace("=", "_").split(" "))
        native.genrule(
...
def cmake_var_string(cmake_vars):
    ...
    """
    return " ".join(
        [_quote("{}={}".format(k, str(v))) for (k, v) in cmake_vars.items()])
...
def expand_cmake_vars(name, src, dst, cmake_vars):
    ...
    Args:
      ...
      cmake_vars: a string containing the CMake variables, as generated by
        cmake_var_string.
    """
    expand_cmake_vars_tool = Label("//:expand_cmake_vars")
    native.genrule(
        name=name,
        srcs=[src],
...
llvm_all_cmake_vars = cmake_var_string(
    _dict_add(
        cmake_vars,
